...
router.post('/leave', auth, async (req, res) => {
  try {
    ...
    if (group.members.length === 0) {
      ...
      await Appointment.deleteMany({ group: group._id });
...
router.delete('/', auth, async (req, res) => {
  try {
    ...
    // Check if user making request is first member
    ...
    await Appointment.deleteMany({ group: groupId });
